...
export const getOptions = async (category) => {
  ...
  const categories = options.reduce((cats, opt) => {
    ...
    if(cat) {
      const tex = cat.textures.find(t => t.name === opt.texture);
      if(tex) {
        tex.options.push(opt);
      } else {
        cat.textures.push({ name: opt.texture, options: [opt] });
      }
    } else {
      cats.push({
        name: opt.category,
        textures: [{ name: opt.texture, options: [opt] }]
      });
    }
  ...
  categories.forEach(cat => {
    cat.textures.sort((a,b) => sorting.textures[a.name] - sorting.textures[b.name]);
  });
